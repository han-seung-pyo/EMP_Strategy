...
#수익률 관찰 포트에 인버스 데이터 추가
def market_inv(num,j):
    temp=0
    for k in range(num):
        temp+=month_date[k]
    c=temp
    d=temp+j
    return alletf['114800'].iloc[c:d]

#수익률 관찰 포트에 레버리지 데이터 추가
def market_lev(num,j):
    temp=0
    for k in range(num):
        temp+=month_date[k]
    c=temp
...
#수익률 관찰 포트에 leverage ETF 데이터 추가
def lev(num,j):
    temp=0
    for k in range(num):
        temp+=month_date[k]
...
#포트폴리오 수익률, 표준편차 구하기
def port_ret_s_m3(w,port,mkt,invetf): ##port=pt:포트폴리오 전체 가격이랑 같이(DF), pf=pf:컬럼 리스트만
    ...
    mkt=mkt.pct_change()
    inve_m=invetf.pct_change().mean()
    inve=invetf.pct_change()
    ret_port_m=list(ret_port_m)
    ret_port_m.append(inve_m)
...
best_port=best()
#best_port.to_excel('all_best_port_return_20180627.xlsx')

for k in range(rebal-1):
    i=best_port['port'][k]
    j=best_port['num'][k]
    mnum=best_port['model'][k]
    pf=best_port['security'][k][0:-1]

    if mnum==2:
        pf=best_port['security'][k][0:-2]
        if len(pf)==0:
            mk=market_inv(13+j,month_date[13+j])
            inv_etf=inv(13+j,month_date[13+j])
            mk=pd.DataFrame(mk)
            inv_etf=pd.DataFrame(inv_etf)
            w=[0]+best_port['weight'][k]  
            rs=port_ret_s_m3(w,mk,mk,inv_etf)
            ...
            inv_etf=inv(13+j,month_date[13+j])
            rs=port_ret_s_m3(w,pr,mk,inv_etf)
            dd_date.append(mk.index[-1])
            mkk=mk_kospi(13+j,month_date[13+j])
            mkm=float(mkk.pct_change().cumsum().tail(1).iloc[0])
            dd_mk.append(mkm)
    ...
    if mnum==3:
        ...
        if len(pf)==0:
            mk=market_inv(13+j,month_date[13+j])
            lev_etf=lev(13+j,month_date[13+j])
            mk=pd.DataFrame(mk)
            lev_etf=pd.DataFrame(lev_etf)
            w=best_port['weight'][k]  
            rs=port_ret_s_m3(w,mk,mk,lev_etf)
            dd_date.append(mk.index[-1])
            mkk=mk_kospi(13+j,month_date[13+j])
            mkm=float(mkk.pct_change().cumsum().tail(1).iloc[0])
            dd_mk.append(mkm)
            ...
            mk=market(13+j,month_date[13+j])
            lev_etf=lev(13+j,month_date[13+j])
            rs=port_ret_s_m3(w,pr,mk,lev_etf)
            dd_date.append(mk.index[-1])
            mkk=mk_kospi(13+j,month_date[13+j])
            mkm=float(mkk.pct_change().cumsum().tail(1).iloc[0])
            dd_mk.append(mkm)
    ...
    if len(pf)==0:
        if mnum==4:
            ...
            continue
        if mnum==5:
            mk=market_lev(13+j,month_date[13+j])
            mk=pd.DataFrame(mk)
            rs=port_ret_s([0.0,1.0],mk,mk)
            dd_date.append(mk.index[-1])
            mkk=mk_kospi(13+j,month_date[13+j])
...
#dd.to_excel('test2_result_20180627.xlsx')
