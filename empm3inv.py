'''
empm3inv.py
내용 : KODEX200 비중 CAP 초과 비중 인버스(114800) ETF로 배분
...
#포트폴리오 수익률, 표준편차 구하기
def port_ret_s(w,port,mkt,invetf): ##port=pt:포트폴리오 전체 가격이랑 같이(DF), pf=pf:컬럼 리스트만
    ...
    for cap in caps:
        for j in range(len(tot_date)):
            ...
            if len(pf)==0:
                ...
                inv_etf=inv(12+j,month_date[12+j])
                mk=pd.DataFrame(mk)
                inv_etf=pd.DataFrame(inv_etf)
                rs=port_ret_s([0.0,1-cap,cap],mk,mk,inv_etf)
                dd=pd.concat([dd,rs])
                sec=list(['114800','069500'])
                ...
                dd_name['weight']=[[1-cap,cap]]
                dd_name_f=pd.concat([dd_name_f,dd_name])
                #print(j,sec,[sharpe(rs,mk)[0]],[1-cap,cap])
                continue
...
dd_final.to_excel('final_empm3inv_20180614.xlsx')