'''
empm3lev.py
내용 : KODEX200 비중 CAP 초과 비중 레버리지(122630) ETF로 배분
...
#포트폴리오 수익률, 표준편차 구하기
def port_ret_s(w,port,mkt,levetf): ##port=pt:포트폴리오 전체 가격이랑 같이(DF), pf=pf:컬럼 리스트만
    ret_port_m=pd.Series(port.pct_change().cumsum().tail(1).iloc[0,:])
    ret_port=port.pct_change()
    mkt_m=float(mkt.pct_change().cumsum().tail(1).iloc[0])
    mkt=mkt.pct_change()
    leve_m=levetf.pct_change().mean()
    leve=levetf.pct_change()
    ret_port_m=list(ret_port_m)
    ret_port_m.append(leve_m)
    ...
    for cap in caps:
        for j in range(len(tot_date)):
            ...
            if len(pf)==0:
                ...
                lev_etf=lev(12+j,month_date[12+j])
                mk=pd.DataFrame(mk)
                lev_etf=pd.DataFrame(lev_etf)
                rs=port_ret_s([0.0,1-cap,cap],mk,mk,lev_etf)
                dd=pd.concat([dd,rs])
                sec=list(['122630','069500'])
...
dd_final.to_excel('final_empm3lev_20180614.xlsx')